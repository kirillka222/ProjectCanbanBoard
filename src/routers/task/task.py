...
from src.crud.task_crud import create_task, get_tasks, get_task, update_task, delete_task
...
def create_new_task(task: TaskCreate, column_id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    return create_task(db, task, column_id)


@router.get("/", response_model=list[TaskOut])
def get_all_tasks(column_id: int, skip: int = 0, limit: int = 10, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
def update_existing_task(task_id: int, task: TaskUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
